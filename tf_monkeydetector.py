"""
A Monkey Detector Interface (as tensorflow code) -- there are two different types of input to handle here:
...
class tfMonkeyDetector(object):
    ...
    def calculateCoMfrom3DJoints(self,jnts):
        # center of mass in 3D space
        com_3d = tf.reduce_mean(jnts, axis=1)
        # project to image coordinates
        com_2d = self.xyztouvd(com_3d)
        return com_2d
    ...
    def xyztouvd(self, jnts_xyz):
        ...
        :return:
        """
        #TODO what happens when a single joint is passed?
        #TODO what if z-value is zero?
        # if jnts_xyz[2] == 0.:
        #     jnt_uvd[0] = self.ux
        #     jnt_uvd[1] = self.uy
        #     return jnt_uvd
        dims = jnts_xyz.shape
        if len(dims) == 2:
            u_s = self.ux - jnts_xyz[:, 0] / jnts_xyz[:, 2] * self.fx
            v_s = self.uy + jnts_xyz[:, 1] / jnts_xyz[:, 2] * self.fy
            d_s = -jnts_xyz[:, 2]
            jnts_uvd = tf.stack([u_s, v_s, d_s], axis=1)
        elif len(dims) == 3:
            u_s = self.ux - jnts_xyz[:, :, 0] / jnts_xyz[:, :, 2] * self.fx
            v_s = self.uy + jnts_xyz[:, :, 1] / jnts_xyz[:, :, 2] * self.fy
            d_s = -jnts_xyz[:, :, 2]
            jnts_uvd = tf.stack([u_s, v_s, d_s], axis=2)
        return jnts_uvd

    def xyztouvd_np(self, jnts_xyz):
        """
        convert from 3d coordinates to image coordinates
        :return:
        """
    ...
    def getAbsoluteCoordinates(self,rel_jnts_xyz,com_uvd):
        ...
        jnts_uvd = self.xyztouvd_np(jnts_xyz)
