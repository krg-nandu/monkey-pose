    ...
    def imgStackDepthOnly(self, seqName, normZeroOne=False):
        ...
        if self._val_prop == None:
            if seqName not in self._imgStacks:
                ...
                labelStack = numpy.zeros((numImgs, j, d), dtype='float32')
                ...
                for i in xrange(numImgs):
                    ...
                    labelStack[i] = numpy.asarray(imgSeq.data[i].gt3Dorig, dtype='float32')

                    #com3DStack[i] = imgSeq.data[i].com
                    #MStack[i] = imgSeq.data[i].T

                    #tmp_label = trans3DsToImg(labelStack[i]*(imgSeq.config['cube'][2] / 2.), com3DStack[i], MStack[i])
            ...
            if seqName not in self._imgStacks:
                ...
                for i in xrange(numImgs):
                    ...
                    if i < train_cut:
                        ...
                        lableStack_v[i - train_cut] = numpy.asarray(imgSeq.data[i].gt3Dorig, dtype='float32')
...
class MonkeyDataset(Dataset):
    def __init__(self, imgSeqs=None,basepath=None,val_prop=None):
        ...
        constructor
        """
        super(MonkeyDataset, self).__init__(imgSeqs,val_prop)
        if basepath is None:
            basepath = '/media/data_cifs/monkey_tracking/'

        self.lmi = MonkeyRendersImporter(basepath)
